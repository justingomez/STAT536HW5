...
Variables that do not appear to have much of an effect include: Standard Deviation of tmean, Diurnal Temp Range, Max Number of Consecutive Dry Days, Highest 5-Day Concentration, Very Wet Days, and Heavy Precip Days. The expected number of significant effects for all of these variables is higher than what we ended up seeing; thus, there is little evidence that these effects are significant in either model. 
