...
\begin{enumerate}
\item %1
Vincent and Mekis are analyzing if there is climate change in Canada by using updated methods of analysis as well as universal practices that are now implemented across the world. Vincent and Mekis want a consistent analysis of climate change in Canada that can be compared or used with data that has been collected around the world. The paper by Vincent and Mekis addresses the links between the temperature and precipitation indices. The first half and second half of the century have probability functions compared.\\
