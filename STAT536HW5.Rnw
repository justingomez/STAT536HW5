...
Thus, we expect roughly 162 of the short trends to be statistically significant and 64 of the longer trends to be significant. However, we see that they are getting a much higher number than this in their runs. 
\\
In both models it appears that Days has the largest difference between the observed number of significant trends and the expected number. This is \\
